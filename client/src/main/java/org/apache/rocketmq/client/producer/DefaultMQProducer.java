...
public class DefaultMQProducer extends ClientConfig implements MQProducer {
     ...
     * 生产者所属组，消息服务器在回查事务状态 时会随机选择该组中任何一个生产者发起的事务回查请求
     * <p>
     ...
     * 自动创建topic的话, 每个默认主题要创建的队列数。
     */
    private volatile int defaultTopicQueueNums = 4;

    /**
     * 默认的发送超时时间，若发送的时候不显示指定timeout，则使用此设置的值作为超时时间。
     * 对于异步发送，超时后会进入回调的onException，对于同步发送，超时则会得到一个RemotingTimeoutException。
     */
    private int sendMsgTimeout = 3000;

    /**
     * 消息体 body 需要压缩的阈值 4K, 消息体超过该值则启用压缩
     * Compress message body threshold, namely, message body larger than 4k will be compressed on default.
     */
    private int compressMsgBodyOverHowmuch = 1024 * 4;

    /**
     * 同步发送失败的话，rocketmq内部重试多少次，默认两次，加上失败一次，一共三次。
     * <p>
    ...
    public void start() throws MQClientException {
        // 使用 namespace 包装 producerGroup
        // 格式 = [%RETRY%/%DLQ%] + [namespace] + % + 资源名称(topic/groupId)
    ...
    public SendResult send(Message msg) throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
        ...
        // 使用 namespace 包装 topic，然后重新设置 topic 名称。
